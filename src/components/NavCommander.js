...
class NavigationCommander extends Component {
  ...
  render () {
    ...
    return (
      <nav className={`navbar navbar-expand-md navbar-dark fixed-top navbar-commander${isMobile ? ' mobile' : '' }`}>
          ...
          <div className={`collapse navbar-collapse ${isOpen ? 'show' : ''}`}>
              ...
              { list.map((i, index) => (
                <Fragment key={i + index}>
                  {i !== 'Installing' && <li className='nav-item'>
                    <NavLink className='nav-link' to={`/docs/commander/slack/${i.toLowerCase().replace(/ /g, '-')}` } onClick={() => googleAnalyticsEvent(8, 'Commander Header', i)}>{i}</NavLink>
                  </li>}
                </Fragment>
              ))}
            </ul>
            ...
            <div className='auth-wrap'>
              <div className='mt-2 mt-md-0 button'>
                <a className='btn btn-outline-light NavLink mr-3' href='/commander/slack/install?version=2' target='_blank' onClick={() => googleAnalyticsEvent(0, 'Commander' +
                  ' Header', 'Add to Slack')} rel='noopener noreferrer nofollow'>Add Commander to Slack</a>
