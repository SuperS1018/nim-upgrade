...
class NavigationCommander extends Component {
  constructor (props) {
    super(props)
  ...
  render () {
    ...
    return (
      <nav className={`navbar navbar-expand-lg navbar-dark fixed-top navbar-commander teams${isMobile ? ' mobile' : '' }`}>
          ...
          <div className={`collapse navbar-collapse ${isOpen ? 'show' : ''}`}>
              ...
              { list.map((i, index) => (
                <Fragment key={i + index}>
                  {i !== 'Installing' && <li className='nav-item'>
                    <NavLink className='nav-link' to={`/docs/commander/microsoft-teams/${i.toLowerCase().replace(/ /g, '-')}` } onClick={() => googleAnalyticsEvent(8, 'Commander Header', i)}>{i}</NavLink>
                  </li>}
                </Fragment>
              ))}
            </ul>
            ...
            <div className='auth-wrap'>
              <div className='mt-2 mt-md-0 button'>
                <a className='btn btn-outline-light mr-3 NavLink' href='https://teams.microsoft.com/l/app/be4751a5-db56-4e45-bb27-397e0aa614b1' target='_blank' rel='noopener noreferrer nofollow' onClick={() => googleAnalyticsEvent(0, 'Commander Header', 'Setup in Microsoft Teams')}>Add Commander to Teams</a>
...
export default NavigationCommander
