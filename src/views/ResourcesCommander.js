...
class ResourcesCommanderSlack extends Component {
  constructor (props) {
    super(props)
    this.state = {
      sidebarSearchTerm: '',
                ...
                {contentList.map((i, index) => (
                  <Fragment key={i.title + index}>
                    <div className='anchor-subject' id={slugifyAnchor(i.anchor || i.title).replace('/', '')}>
                      {/(?=.*resources)(?=.*video)/i.test(window.location.pathname) ? <h2 className='mb-3' style={{textIndent: '0'}}>{i.title}</h2> : <h1>{i.title}</h1>}
...
export default withRouter(ResourcesCommanderSlack)
