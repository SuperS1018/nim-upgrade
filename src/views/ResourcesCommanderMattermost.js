...
import NavCommanderMattermost from '../components/NavCommanderMattermost'
import { slugify, slugifyAnchor } from '../util/url'
import { isCMS } from '../util/misc'
import Scrollspy from 'react-scrollspy'
import '../stylesheets/views/Resources.css'
import '../stylesheets/views/ResourcesCommanderMattermost.css'
...
class ResourcesCommanderMattermost extends Component {
  constructor (props) {
    super(props)
    this.state = {
      sidebarSearchTerm: '',
  ...
  componentDidMount () {
    if(!/admin/.test(window.location.pathname)) {
      ...
      // console.log(action, pathname, hash, this.props.history, key)
      if(hash) {
  ...
  render () {
    ...
    const { fields: { title }, fields, list, history } = this.props
    ...
    return (
      <div className='section pb-commander'>
        <NavCommanderMattermost
          list={navList}
          history={history}
          ...
          showNext={showNext}
        />
        <main className='Resources Commander flex-grow-1' id={`page-${pagename}`}>
                ...
                {contentList.map((i, index) => (
                  <Fragment key={i.title + index}>
                    <div className='anchor-subject' id={slugifyAnchor(i.anchor || i.title).replace('/', '')}>
                      ...
                      <Content source={i.body} className={`${slugifyAnchor(title)}`} container={'containerElement'} />
                    </div>
                    ...
                    i['anchor-item'].map((item, idx) => (
                      <div id={slugifyAnchor(item.anchor || item.title).replace('/', '')} key={item.title + idx}>
                        <h2>{item.title}</h2>
                        <Content source={item.body} className={`${slugifyAnchor(title)}`} container={'containerElement'} />
...
export default withRouter(ResourcesCommanderMattermost)
